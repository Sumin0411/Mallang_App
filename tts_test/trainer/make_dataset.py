...
def make_test_metadata(root_path, save_path):
    ...
    items = formatter(root_path)
    with open(save_path, "w", encoding="utf-8") as f:
        for item in items[:4300]:
            f.write("wavs/" + item["audio_file"] + "|" + item["text"] + "|" + item["speaker_name"] + "\n")
    print(f"Saved to {save_path}")
    
make_metadata("C:\\lee\\133.감성 및 발화 스타일 동시 고려 음성합성 데이터\\01-1.정식개방데이터\\Training\\02.라벨링데이터", "C:\\lee\\133.감성 및 발화 스타일 동시 고려 음성합성 데이터\\01-1.정식개방데이터\\Training\\metadata_train.txt")
make_test_metadata("C:\\lee\\133.감성 및 발화 스타일 동시 고려 음성합성 데이터\\01-1.정식개방데이터\\Training\\02.라벨링데이터", "C:\\lee\\133.감성 및 발화 스타일 동시 고려 음성합성 데이터\\01-1.정식개방데이터\\Training\\metadata_test.txt")